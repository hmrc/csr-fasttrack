...
trait GeneralApplicationRepository {
  ...
  def updateStatus(applicationId: String, status: ApplicationStatuses.ApplicationStatus): Future[Unit]
...
class GeneralApplicationMongoRepository(timeZoneService: TimeZoneService)(implicit mongo: () => DB)
  ...
  def findStatusDetails(applicationId: String): Future[ApplicationStatusDetails] = {
    ...
    findProgress(applicationId).flatMap { progress =>
      ...
      collection.find(query, projection).one[BSONDocument] map {
        case Some(document) =>
          val applicationStatus = document.getAs[ApplicationStatuses.ApplicationStatus]("applicationStatus").get
